...
class DocumentationManager:
    ...
    def _create_docs_structure(self):
        ...
        try:
            ...
            self.success_docs_dir = self.docs_dir / "✅_EXITOSOS"
            self.failed_docs_dir = self.docs_dir / "❌_FALLIDOS"
            self.partial_docs_dir = self.docs_dir / "⚠️_PARCIALES"
            self.unknown_docs_dir = self.docs_dir / "❓_DESCONOCIDOS"
...
- **🗓️ Fecha**: {exec_info.get('execution_date', 'N/A')}
- **⏰ Hora de Inicio**: {exec_info.get('start_time', 'N/A')}
- **⏱️ Duración Total**: {exec_info.get('total_duration', 'N/A')}
...
"""
        ...
        for i, step in enumerate(steps, 1):
            status_emoji = "✅" if step.get('status') == 'SUCCESS' else "❌" if step.get('status') == 'FAILED' else "⚠️"
            content += f"""### {i}. {status_emoji} {step.get('name', 'Paso sin nombre')}

- **📄 Descripción**: {step.get('description', 'Sin descripción')}
- **⏱️ Duración**: {step.get('duration', 'No disponible')}
