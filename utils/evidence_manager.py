...
class EvidenceManager:
    ...
    def __init__(self, config_file="config.json"):
        ...
        self.retention_days = self.config.get("evidence_management", {}).get(
            "retention_days", 30
        )
        self.archive_after_days = self.config.get("evidence_management", {}).get(
            "archive_after_days", 90
        )
        self.cleanup_temp_files = self.config.get("evidence_management", {}).get(
            "cleanup_temp_files", True
        )
        self.generate_daily_summaries = self.config.get("evidence_management", {}).get(
            "generate_daily_summaries", True
        )
        self.max_screenshots_per_execution = self.config.get(
            "evidence_management", {}
        ).get("max_screenshots_per_execution", 20)
        self.compress_old_evidence = self.config.get("evidence_management", {}).get(
            "compress_old_evidence", True
        )

        self.logger.info(
            f"EvidenceManager inicializado - Retención: {self.retention_days} días, Archivado: {self.archive_after_days} días"
        )
    ...
    def _load_config(self, config_file):
        ...
        try:
            ...
            logging.warning(
                f"No se pudo cargar {config_file}, usando configuración por defecto: {str(e)}"
            )
            return {}

    def create_evidence_structure(
        self, feature_name, scenario_name, execution_result="UNKNOWN"
    ):
    ...
    def create_execution_folder(
        ...
        """Crea una carpeta específica para cada ejecución de prueba"""
        try:
            if execution_timestamp is None:
                execution_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

            # Crear estructura: evidences/YYYY-MM-DD/feature_name/scenario_name/execution_timestamp/
            ...
            self.logger.info(f"📁 Carpeta de ejecución creada: {execution_folder}")
            return execution_folder

        except Exception as e:
            self.logger.error(f"❌ Error creando carpeta de ejecución: {e}")
            # Fallback a estructura simple
            return self._create_simple_structure(feature_name, scenario_name)[0]

    def get_execution_folder(
        self, feature_name, scenario_name, execution_timestamp=None
    ):
        """Obtiene la carpeta de ejecución actual o crea una nueva"""
        try:
            if execution_timestamp is None:
                execution_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            self.logger.error(f"❌ Error obteniendo carpeta de ejecución: {e}")
            # Fallback a estructura simple
            return self._create_simple_structure(feature_name, scenario_name)[0]
    ...
    def cleanup_old_evidence(self):
        ...
        try:
            ...
            self.logger.info(
                f"Limpieza completada - Limpiados: {cleaned_count}, Archivados: {archived_count}"
            )
    ...
    def _limit_screenshots(self, screenshots_dir):
        ...
        try:
            ...
            if len(screenshots) > self.max_screenshots_per_execution:
                ...
                self.logger.info(
                    f"Eliminados {len(to_remove)} screenshots antiguos de {screenshots_dir}"
                )

        except Exception as e:
            self.logger.error(
                f"Error limitando screenshots en {screenshots_dir}: {str(e)}"
            )
                    ...
                    feature_summary["execution_details"].append(
                        {
                            "name": execution_name,
                            "timestamp": (
                                execution_name.split("_")[-1]
                                if "_" in execution_name
                                else "unknown"
                            ),
                            "screenshots_count": len(
                                list((execution_dir / "screenshots").glob("*.png"))
                                if (execution_dir / "screenshots").exists()
                                else []
                            ),
                        }
                    )
